    ...
    endpoints.forEach(ep => {
      socket.on(`endpoint_update_${ep.id}`, (result: EndpointUpdate) => {
        setLiveData(prev => {
          ...
          const newData = [...current, { name: time, latency: result.responseTime, status: result.statusCode }];
    ...
    <div className="animate-fade-in">
      <div className="page-header flex-between">
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '4px' }}>Welcome back, {user?.email}</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
        <div className="card">
          <h3 style={{ marginBottom: '16px', color: 'var(--text-secondary)' }}>Total Endpoints</h3>
          <div style={{ fontSize: '2.5rem', fontWeight: '700' }}>{endpoints.length}</div>
        </div>
        <div className="card">
          <h3 style={{ marginBottom: '16px', color: 'var(--text-secondary)' }}>Live Tracking</h3>
        ...
        return (
          <div key={ep.id} className="card" style={{ marginTop: '32px' }}>
            <div className="flex-between" style={{ marginBottom: '24px' }}>
              <h3>{ep.name} <span style={{fontSize: '0.9rem', color: 'var(--text-secondary)'}}>({ep.url})</span></h3>
              ...
              <div style={{ height: '300px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data}>
                    ...
                    />
                    <Line type="monotone" dataKey="latency" name="Latency (ms)" stroke="var(--primary)" strokeWidth={3} dot={{r: 4, fill: 'var(--bg-surface)', strokeWidth: 2}} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
